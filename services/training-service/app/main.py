...
EXPERIMENT_NAME = os.getenv(
    ...
    "cyber-intrusion-model-selection",
)
...
def get_skops_trusted_types(model):
    """
    The model was trained locally by this service.
    Discover third-party model types that skops requires
    to be explicitly trusted before MLflow serializes it.
    """
    ...
    with tempfile.NamedTemporaryFile(
        ...
        return skops.io.get_untrusted_types(
            file=tmp.name
        )
...
def train(
    ...
    if runtime.running:
        ...
        raise HTTPException(
            409,
            "Training suite is already running.",
        )
